...
bool initFramegen(const char *cacheDir) {
    ...
    if (useFp16) {
        LOGI("A6XX shader route=SPIRV-FP16 (Lossless.dll resource IDs 304..351)");
    } else if (useFp32Spirv) {
        LOGI("A6XX shader route=SPIRV-FP32 (Lossless.dll resource IDs 353..400) "
             "— bypasses dxvk DXBC translator (deviceVMM=%d)", (int)deviceHasVMM);
    } else {
        LOGI("A6XX shader route=DXBC-TRANSLATED (Lossless.dll resource IDs 255..302) "
    ...
    try {
        ...
        LOGI("A6XX LSFG initialize result=SUCCESS");
        return true;
    } catch (const std::exception &e) {
        LOGE("A6XX LSFG initialize result=FAILED reason=%s; capture-only selected", e.what());
        return false;
    } catch (...) {
...
bool createFramegenContext() {
    ...
    try {
        ...
        LOGE("A6XX createContextFromAHB result=FAILED reason=%s", e.what());
        return false;
    }
    LOGI("A6XX createContextFromAHB result=SUCCESS contextId=%d", g.framegenCtxId);
    return true;
}
...
int initRenderLoop(const char *cacheDir, const RenderLoopConfig &cfg) {
    ...
    LOGI("A6XX framegen enabled=%s reason=%s",
         g.framegenCtxId >= 0 ? "YES" : "NO",
         g.framegenCtxId >= 0 ? "all required capabilities and operations succeeded"
                              : "LSFG initialize or createContextFromAHB failed; see preceding exact error");
