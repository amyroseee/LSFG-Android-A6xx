...
// Required by the AHardwareBuffer path. The Android framegen API passes AHBs
// directly; Linux opaque-FD memory/semaphore extensions are not prerequisites.
...
VkInstance make_instance(bool &hasSurfaceExts) {
    ...
    const VkApplicationInfo appInfo{
        ...
        .apiVersion = VK_API_VERSION_1_1,  // AHB path supports Vulkan 1.1 devices
    };
...
VkPhysicalDevice pick_physical_device(VkInstance inst) {
    ...
    for (auto d : devs) {
        ...
        if (props.apiVersion < VK_API_VERSION_1_1) {
            LOGW("GPU rejected for AHB path: Vulkan %u.%u is below required 1.1",
        ...
        for (const char *req : kRequiredDeviceExt) {
            if (!has_extension(exts, req)) {
                LOGW("GPU rejected for AHB path: missing required extension %s", req);
                ok = false;
            }
        }
        if (ok) return d;
...
int create_session(VulkanSession &out) {
    ...
    for (const char *opt : kOptionalDeviceExt) {
        if (has_extension(avail, opt)) {
            ...
            } else if (std::strcmp(opt, VK_KHR_EXTERNAL_MEMORY_FD_EXTENSION_NAME) == 0) {
                out.hasExternalMemoryFd = true;
            }
        } else {
    ...
    // Timeline semaphores are optional. The AHB/framegen path uses binary
    // semaphores between prepass and generation passes plus per-slot fences.
    // Preserve timeline when advertised, but never request an absent extension.
    bool timelineViaKhr = false;
    if (!api12) {
        if (has_extension(avail, "VK_KHR_timeline_semaphore")) {
            enabledExts.push_back("VK_KHR_timeline_semaphore");
            timelineViaKhr = true;
        } else {
            LOGI("VK_KHR_timeline_semaphore absent — using fence/binary fallback");
        }
    }
    ...
    VkPhysicalDeviceTimelineSemaphoreFeaturesKHR timelineQuery{
        .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_TIMELINE_SEMAPHORE_FEATURES_KHR,
        .pNext = &memModelQuery,
    };
    VkPhysicalDeviceSamplerYcbcrConversionFeatures ycbcrQuery{
        .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SAMPLER_YCBCR_CONVERSION_FEATURES,
        .pNext = (api12 || timelineViaKhr)
            ? static_cast<void*>(&timelineQuery)
            : static_cast<void*>(&memModelQuery),
    };
    VkPhysicalDeviceRobustness2FeaturesEXT robustnessQuery{
        .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ROBUSTNESS_2_FEATURES_EXT,
        .pNext = &ycbcrQuery,
    };
    VkPhysicalDeviceFeatures2 features2Query{
        .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2,
        .pNext = out.hasRobustness2
            ? static_cast<void*>(&robustnessQuery)
            : static_cast<void*>(&ycbcrQuery),
    };
    vkGetPhysicalDeviceFeatures2(out.physicalDevice, &features2Query);
    const bool hasMemModel = memModelQuery.vulkanMemoryModel;
    out.hasTimelineSemaphore = (api12 || timelineViaKhr)
        && timelineQuery.timelineSemaphore == VK_TRUE;
    out.hasRobustness2 = out.hasRobustness2
        && robustnessQuery.nullDescriptor == VK_TRUE;
    if (ycbcrQuery.samplerYcbcrConversion != VK_TRUE) {
        LOGE("AHB capability failed: samplerYcbcrConversion feature unavailable");
        return kSessionNoSuitableGpu;
    }
    ...
    VkPhysicalDeviceTimelineSemaphoreFeaturesKHR timelineSem{
        ...
        .timelineSemaphore = out.hasTimelineSemaphore ? VK_TRUE : VK_FALSE,
    };
    VkPhysicalDeviceSamplerYcbcrConversionFeatures ycbcr{
        .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SAMPLER_YCBCR_CONVERSION_FEATURES,
        .pNext = out.hasTimelineSemaphore
            ? static_cast<void*>(&timelineSem)
            : (enableSync2 ? static_cast<void*>(&sync2) : static_cast<void*>(&memModel)),
    ...
    LOGI("A6XX caps: AHB=YES FD path required=NO FD export=%s robustness2=%s "
         "nullDescriptor=%s timeline=%s sync2=%s fallbackSync=YES computeQueue=YES",
         out.hasExternalMemoryFd ? "OPTIONAL-AVAILABLE" : "UNAVAILABLE",
         out.hasRobustness2 ? "YES" : "NO",
         out.hasRobustness2 ? "REAL" : "FALLBACK",
         out.hasTimelineSemaphore ? (api12 ? "NATIVE" : "KHR") : "FENCE-BINARY-FALLBACK",
         enableSync2 ? (api13 ? "NATIVE" : "KHR") : "LEGACY-BARRIER-FALLBACK");
    LOGI("Vulkan session ready (GPU=%s API=%u.%u.%u compute family=%u, %zu extensions, swapchain=%s)",
