...
Java_com_lsfg_android_session_NativeBridge_initContext(
    ...
    // Accept any complete route. Requiring DXBC id 255 here incorrectly blocked
    // devices that intentionally use the precompiled FP32/FP16 SPIR-V route to
    // avoid VulkanMemoryModel emitted by the DXBC translator.
    const bool hasDxbc = !lsfg_android::load_cached_spirv(cache, 255).empty();
    const bool hasFp32 = lsfg_android::fp32_spirv_shaders_available(cache);
    const bool hasFp16 = lsfg_android::fp16_shaders_available(cache);
    if (!hasDxbc && !hasFp32 && !hasFp16) {
